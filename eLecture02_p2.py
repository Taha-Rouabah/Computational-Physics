...
"""
@author: rouabah
This program contains solution of Exercises 2.1 and
"""
...
# Data
x = np.array([1.0, 2.0, 4.0, 6.0])
x_bis = np.linspace(x[0], x[-1], 50)    # this will be used to generate smooth curves
y = np.array([0.406006, 0.0549469, 0.00100039, 0.0000184326])
log_y = np.log(y)
...
fit1, fit1_cov = curve_fit(exponential, x, y)
y_fit1 = exponential(x, fit1[0], fit1[1])
y_fit1_bis = exponential(x_bis, fit1[0], fit1[1]) # this will give a smooth curve

# linear fit of ln(y) vs x data
fit2, fit2_cov = curve_fit(linear, x, log_y)
y_fit2 = linear(x, fit2[0], fit2[1])
y_fit2_bis = linear(x_bis, fit2[0], fit2[1])
"""
...
plt.xlabel('x')
plt.ylabel('log(y)')
plt.title('log(y) vs x (semi-log plot)')
plt.show()
...
R2_fit1 = R2(y_fit1, y)
print('The R2 given by the exponential fit is {}'.format(R2_fit1))
R2_fit2 = R2(y_fit2, log_y)
print('The R2 given by the linear fit is {}'.format(R2_fit2))
...
the result will be of the order of 10^(-6) means that the covariance matrices
are the same.
"""
